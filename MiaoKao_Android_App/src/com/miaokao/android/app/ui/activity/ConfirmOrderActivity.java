...
public class ConfirmOrderActivity extends BaseActivity implements OnClickListener, LoginStatusListenner {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mDrivingSchool = (DrivingSchool) bundle.getSerializable("drivingSchool");
		mDSchoolCourse = (DSchoolCourse) bundle.getSerializable("schoolCourse");
	...
	private void initDate() {
		...
		mP_NameTxt.setText(mDSchoolCourse.getP_type());
		mP_TypeTxt.setText(mDSchoolCourse.getP_name());
		String p_price = mDSchoolCourse.getP_price();
		if(!TextUtils.isEmpty(mDSchoolCourse.getDiscount_price())) {
			p_price = mDSchoolCourse.getDiscount_price();
		}
		// 培训费用
		mP_PriceTxt.setText(PubUtils.moneyFormat(p_price) + "元");
		// 备注
		mInfoTxt.setText(mDSchoolCourse.getP_intro());
		// 总计金额
		try {
		...
		// 学生优化金额
		try {
			double temp = Double.parseDouble(mDSchoolCourse.getStu_price());
			mStuPrice = (int) temp;
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		...
		// 首付款
		try {
			double pay_rate = Double.parseDouble(mDrivingSchool.getFirst_pay_rate());
			// 比例
	...
	public void onClick(View v) {
		...
		case R.id.confirm_order_contact_txt:
			...
			break;
		case R.id.confirm_select_coach_name_layout:
			// 选择教练
			intent =new Intent(mContext, CoachActivity.class);
			intent.putExtra("isSelectCoach", true);
			intent.putExtra("mer_id", mDrivingSchool.getMer_account());
			startActivityForResult(intent, PubConstant.SELECT_COACH_NAME_CODE);
			break;
	...
	private void goPay(Map<String, String> postData, String id) {
		...
		order.setMer_id(mDSchoolCourse.getP_mer_id());	// 驾校帐号
	...
	protected void submitOrder() {
		...
		postData.put("coach_id", mCoach_id);	// 教练ID
		postData.put("mer_id", mDrivingSchool.getMer_account());
		postData.put("mer_type", "1"); // 商家类型，1表示商家，2表示个人
		postData.put("mer_name", mDrivingSchool.getMer_name());
		postData.put("product_id", mDSchoolCourse.getId());
		postData.put("product_name", mDSchoolCourse.getP_name());
		postData.put("product_price", mDSchoolCourse.getP_price());
		postData.put("product_num", "");
		postData.put("class_level", mDSchoolCourse.getP_type());
